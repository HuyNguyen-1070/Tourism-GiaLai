...
export const VerificationCode = ({ email, onVerified, onBack }: VerificationCodeProps) => {
  ...
  const handlePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    if (paste) {
      const newCode = Array(VERIFICATION_CODE_LENGTH).fill('');
      paste.split('').forEach((char, i) => {
        newCode[i] = char;
      });
      setCode(newCode);
      const nextEmpty =
        paste.length < VERIFICATION_CODE_LENGTH ? paste.length : VERIFICATION_CODE_LENGTH - 1;
    ...
    <div className="w-full space-y-6">
      ...
      <div className="text-center space-y-3">
        ...
        <p className="font-body-md text-on-surface-variant">
          Mã xác thực đã được gửi đến <span className="font-semibold text-on-surface">{email}</span>
            ...
            className={`w-full aspect-square text-center font-headline-md text-headline-md rounded-lg border-2 transition-all outline-none
              ${
                error
                  ...
                  : digit
                    ? 'border-forest-leaf bg-primary-fixed/20'
                    : 'border-outline-variant bg-surface-container-low focus:border-forest-leaf focus:ring-2 focus:ring-forest-leaf/20'
              }`}
          />
        ))}
      </div>
      ...
      <button
        ...
        className="w-full py-4 px-6 bg-forest-leaf text-white font-label-md text-label-md rounded-lg shadow-sm hover:bg-primary transition-all active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {isVerifying ? (
          <>
