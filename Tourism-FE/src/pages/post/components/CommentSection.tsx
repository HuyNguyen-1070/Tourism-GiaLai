...
export const CommentSection = ({ postId }: CommentSectionProps) => {
    ...
    const { data, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteQuery({
        ...
        queryFn: ({ pageParam = 0 }) => interactionApi.getComments(postId, { page: pageParam, size: 10 }),
    ...
    return (
        <section className="mt-12 pt-8 border-t border-outline-variant/20">
            ...
            {user && <CommentForm onSubmit={(content) => addMutation.mutate(content)} isSubmitting={addMutation.isPending} />}
            ...
            {isLoading ? (
                ...
                <div className="space-y-6">
                    ...
                    {comments.length === 0 && <p className="text-on-surface-variant text-center py-8">Chưa có bình luận nào. Hãy là người đầu tiên!</p>}
                </div>
            )}
        </section>
    );
