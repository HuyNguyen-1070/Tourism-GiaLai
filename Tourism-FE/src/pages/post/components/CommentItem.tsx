            ...
            <div className="flex-1 space-y-1">
                ...
                {isEditing ? (
                    <div className="space-y-3">
                        <Textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} rows={3} className="w-full" />
                ...
                {(isOwner || isAdmin) && !isEditing && (
                    <div className="flex gap-3 mt-2">
                        {isOwner && (
                            <button onClick={() => setIsEditing(true)} className="text-xs text-secondary hover:underline flex items-center gap-1">
                                <Pencil size={12} /> Sửa
                            </button>
                        )}
                        <button onClick={() => deleteMutation.mutate()} className="text-xs text-error hover:underline flex items-center gap-1">
                            <Trash2 size={12} /> Xóa
                        </button>
                    </div>
                )}
